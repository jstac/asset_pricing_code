...
def test_lininterp():
    ...
    x_grid = np.linspace(-1, 1, nx)
    y_grid = np.linspace(-1, 1, ny)
    z_grid = np.linspace(-1, 1, nz)
    vals = np.empty((nx, ny, nz))

    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                vals[i, j, k] = f3(x_grid[i], y_grid[j], z_grid[k])

    for q in range(10):
        t = np.random.randn(3)
        p = f3(*t)
        p_hat = lininterp_3d(x_grid, y_grid, z_grid, vals, t)
        assert np.allclose(p, p_hat)

    @njit
    def f4(u, v, w, x):
        return intercept + a * u + b * v + c * w + d * x 

    nu, nv, nw, nx = 10, 10, 10, 10
    u_grid = np.linspace(-1, 1, nu)
    v_grid = np.linspace(-1, 1, nv)
    w_grid = np.linspace(-1, 1, nw)
    x_grid = np.linspace(-1, 1, nx)
    vals = np.empty((nu, nv, nw, nx))
    ...
    for i in range(nu):
        for j in range(nv):
            for k in range(nw):
                for l in range(nx):
                    vals[i, j, k, l] = \
                        f4(u_grid[i], v_grid[j], w_grid[k], x_grid[l])

    for q in range(10):
        t = np.random.randn(4)
        p = f4(*t)
        p_hat = lininterp_4d(u_grid, v_grid, w_grid, x_grid, vals, t)
        assert np.allclose(p, p_hat)
